...
-opaque builder(RCO, RCP) :: {builder,
        integer(),
        fun((gleam@http@request:request(RCO)) -> gleam@http@response:response(RCP)),
...
-type websocket_message(RCQ) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, RCQ}.
...
-spec new(
    fun((gleam@http@request:request(RDL)) -> gleam@http@response:response(RDN))
) -> builder(RDL, RDN).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(RDR, RDS), integer()) -> builder(RDR, RDS).
port(Builder, Port) ->
    erlang:setelement(2, Builder, Port).

-spec read_request_body(
    builder(bitstring(), RDX),
    integer(),
    gleam@http@response:response(RDX)
) -> builder(mist@internal@http:connection(), RDX).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(REV)
) -> {ok, websocket_message(REV)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((RFB, mist@internal@websocket:websocket_connection(), websocket_message(RFC)) -> gleam@otp@actor:next(RFC, RFB)),
    fun((mist@internal@websocket:websocket_connection()) -> {RFB,
        gleam@option:option(gleam@erlang@process:selector(RFC))}),
    fun((RFB) -> nil)
...
-spec server_sent_events(
    ...
    fun(() -> gleam@otp@actor:init_result(RFQ, RFR)),
    fun((RFR, sse_connection(), RFQ) -> gleam@otp@actor:next(RFR, RFQ))
) -> gleam@http@response:response(response_data()).
