...
function set_child(comp, child_comp) {
  const elem = get_element(comp);
...
function add_reference(ref, component2, type) {
...
function read_reference(ref) {
  const { type, component: component2 } = globalThis.reference_map.get(ref.id);
  const elem = get_element(component2);
  if (type === "InnerHTML") {
...
var InnerHTML = class extends CustomType {
};
...
function set(ref, ref_type) {
  return new ComponentParameterList(
    (component2) => {
      add_reference(ref, component2, ref_type);
      return toList([]);
    }
  );
}
...
function main() {
  return start(
    () => {
      let ref = create();
      return div(
        toList([class$("p-5 bg-blue-100"), set(ref, new InnerHTML())]),
        toList([
          div(
            toList([
