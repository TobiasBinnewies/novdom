...
-opaque cli(WWW) :: {cli,
        fun((env()) -> {env(),
            {ok, WWW} | {error, lustre_dev_tools@error:error()}})}.
...
-spec run(cli(WWX), glint:flags()) -> {ok, WWX} |
...
-spec from_result({ok, WXF} | {error, lustre_dev_tools@error:error()}) -> cli(WXF).
from_result(Result) ->
    {cli, fun(Env) -> {Env, Result} end}.

-spec do(cli(WXJ), fun((WXJ) -> cli(WXL))) -> cli(WXL).
do(Step, Next) ->
    {cli,
        fun(Env) ->
            {Env@1, Result} = (erlang:element(2, Step))(Env),
...
-spec in(fun(() -> WXO)) -> cli(WXO).
in(Value) ->
    {cli, fun(Env) -> {Env, {ok, Value()}} end}.

-spec map(cli(WXQ), fun((WXQ) -> WXS)) -> cli(WXS).
map(Step, Next) ->
    {cli,
        fun(Env) ->
            {Env@1, Result} = (erlang:element(2, Step))(Env),
...
-spec 'try'(
    {ok, WXU} | {error, lustre_dev_tools@error:error()},
    fun((WXU) -> cli(WXX))
) -> cli(WXX).
'try'(Result, Next) ->
...
-spec log(binary(), fun(() -> cli(WYA))) -> cli(WYA).
log(Message, Next) ->
    {cli,
        fun(Env) ->
...
-spec success(binary(), fun(() -> cli(WYD))) -> cli(WYD).
...
-spec notify(binary(), fun(() -> cli(WYG))) -> cli(WYG).
notify(Message, Next) ->
    {cli,
        fun(Env) ->
            Env@1 = erlang:setelement(3, Env, case erlang:element(3, Env) of
...
-spec template(binary(), fun((binary()) -> cli(WYL))) -> cli(WYL).
template(Name, Next) ->
    {cli,
        fun(Env) ->
            _assert_subject = gleam_erlang_ffi:priv_directory(
                <<"novdom_dev_tools"/utf8>>
            ),
...
-spec get_config_value(
    ...
    WYR,
    list(binary()),
    fun((gleam@dict:dict(binary(), tom:toml()), list(binary())) -> {ok, WYR} |
        {error, any()}),
    fun((glint:flags()) -> {ok, WYR} | {error, any()})
) -> cli(WYR).
