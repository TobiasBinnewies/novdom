...
-spec equal(LPE, LPE) -> nil.
equal(A, B) ->
    gleeunit_ffi:should_equal(A, B).

-spec not_equal(LPF, LPF) -> nil.
not_equal(A, B) ->
    gleeunit_ffi:should_not_equal(A, B).

-spec be_ok({ok, LPG} | {error, any()}) -> LPG.
be_ok(A) ->
    gleeunit_ffi:should_be_ok(A).

-spec be_error({ok, any()} | {error, LPL}) -> LPL.
be_error(A) ->
    gleeunit_ffi:should_be_error(A).

-spec be_some(gleam@option:option(LPO)) -> LPO.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
