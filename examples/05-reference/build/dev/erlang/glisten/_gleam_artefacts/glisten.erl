...
-type message(ODB) :: {packet, bitstring()} | {user, ODB}.

-type connection(ODC) :: {connection,
        {ok, {{integer(), integer(), integer(), integer()}, integer()}} |
            {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(ODC))}.

-opaque handler(ODD, ODE) :: {handler,
        fun((connection(ODD)) -> {ODE,
            gleam@option:option(gleam@erlang@process:selector(ODD))}),
        fun((message(ODD), ODE, connection(ODD)) -> gleam@otp@actor:next(message(ODD), ODE)),
        gleam@option:option(fun((ODE) -> nil)),
        integer()}.
...
-spec convert_on_init(
    fun((connection(OEB)) -> {OED,
        gleam@option:option(gleam@erlang@process:selector(OEB))})
) -> fun((glisten@internal@handler:connection(OEB)) -> {OED,
    gleam@option:option(gleam@erlang@process:selector(OEB))}).
...
-spec handler(
    fun((connection(OEJ)) -> {OEL,
        gleam@option:option(gleam@erlang@process:selector(OEJ))}),
    fun((message(OEJ), OEL, connection(OEJ)) -> gleam@otp@actor:next(message(OEJ), OEL))
) -> handler(OEJ, OEL).
handler(On_init, Loop) ->
    {handler, On_init, Loop, none, 10}.

-spec map_user_selector(gleam@erlang@process:selector(message(ODQ))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(ODQ)).
...
-spec convert_loop(
    fun((message(ODV), ODW, connection(ODV)) -> gleam@otp@actor:next(message(ODV), ODW))
) -> fun((glisten@internal@handler:loop_message(ODV), ODW, glisten@internal@handler:connection(ODV)) -> gleam@otp@actor:next(glisten@internal@handler:loop_message(ODV), ODW)).
convert_loop(Loop) ->
    fun(Msg, Data, Conn) ->
        Conn@1 = {connection,
            erlang:element(2, Conn),
...
-spec with_close(handler(OES, OET), fun((OET) -> nil)) -> handler(OES, OET).
with_close(Handler, On_close) ->
    erlang:setelement(4, Handler, {some, On_close}).

-spec with_pool_size(handler(OEY, OEZ), integer()) -> handler(OEY, OEZ).
