...
-spec push_key({ok, UNQ} | {error, get_error()}, binary()) -> {ok, UNQ} |
    {error, get_error()}.
push_key(Result, Key) ->
    case Result of
        {ok, T} ->
...
-spec expect_end_of_line(
    list(binary()),
    fun((list(binary())) -> {ok, {UPV, list(binary())}} | {error, parse_error()})
) -> {ok, {UPV, list(binary())}} | {error, parse_error()}.
...
-spec do(
    {ok, {UQG, list(binary())}} | {error, parse_error()},
    fun((UQG, list(binary())) -> {ok, UQJ} | {error, parse_error()})
) -> {ok, UQJ} | {error, parse_error()}.
do(Result, Next) ->
    case Result of
        {ok, {A, Input}} ->
            Next(A, Input);
...
-spec expect(
    list(binary()),
    binary(),
    fun((list(binary())) -> {ok, {UQO, list(binary())}} | {error, parse_error()})
) -> {ok, {UQO, list(binary())}} | {error, parse_error()}.
