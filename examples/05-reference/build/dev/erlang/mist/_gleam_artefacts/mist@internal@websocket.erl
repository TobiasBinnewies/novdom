...
-type valid_message(SBH) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, SBH}.

-type websocket_message(SBI) :: {valid, valid_message(SBI)} | invalid.
...
-type handler_message(SBJ) :: {internal, gramps@websocket:frame()} | {user, SBJ}.

-type websocket_state(SBK) :: {websocket_state,
        bitstring(),
        SBK,
...
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(SCS))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(SCS))).
...
-spec apply_frames(
    ...
    fun((SCI, websocket_connection(), handler_message(SCJ)) -> gleam@otp@actor:next(SCJ, SCI)),
    websocket_connection(),
    gleam@otp@actor:next(websocket_message(SCJ), SCI),
    fun((SCI) -> nil)
) -> gleam@otp@actor:next(websocket_message(SCJ), SCI).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {SBT,
        gleam@option:option(gleam@erlang@process:selector(SBU))}),
    fun((SBT) -> nil),
    fun((SBT, websocket_connection(), handler_message(SBU)) -> gleam@otp@actor:next(SBU, SBT)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok, gleam@erlang@process:subject(websocket_message(SBU))} | {error, nil}.
