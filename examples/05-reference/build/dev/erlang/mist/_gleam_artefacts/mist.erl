...
-opaque builder(SOC, SOD) :: {builder,
        integer(),
        fun((gleam@http@request:request(SOC)) -> gleam@http@response:response(SOD)),
...
-type websocket_message(SOE) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, SOE}.
...
-spec new(
    fun((gleam@http@request:request(SOZ)) -> gleam@http@response:response(SPB))
) -> builder(SOZ, SPB).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(SPF, SPG), integer()) -> builder(SPF, SPG).
port(Builder, Port) ->
    erlang:setelement(2, Builder, Port).

-spec read_request_body(
    builder(bitstring(), SPL),
    integer(),
    gleam@http@response:response(SPL)
) -> builder(mist@internal@http:connection(), SPL).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(SQJ)
) -> {ok, websocket_message(SQJ)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((SQP, mist@internal@websocket:websocket_connection(), websocket_message(SQQ)) -> gleam@otp@actor:next(SQQ, SQP)),
    fun((mist@internal@websocket:websocket_connection()) -> {SQP,
        gleam@option:option(gleam@erlang@process:selector(SQQ))}),
    fun((SQP) -> nil)
...
-spec server_sent_events(
    ...
    fun(() -> gleam@otp@actor:init_result(SRE, SRF)),
    fun((SRF, sse_connection(), SRE) -> gleam@otp@actor:next(SRF, SRE))
) -> gleam@http@response:response(response_data()).
