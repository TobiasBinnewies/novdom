...
-opaque cli(XLT) :: {cli,
        fun((env()) -> {env(),
            {ok, XLT} | {error, lustre_dev_tools@error:error()}})}.
...
-spec run(cli(XLU), glint:flags()) -> {ok, XLU} |
...
-spec from_result({ok, XMC} | {error, lustre_dev_tools@error:error()}) -> cli(XMC).
from_result(Result) ->
    {cli, fun(Env) -> {Env, Result} end}.

-spec do(cli(XMG), fun((XMG) -> cli(XMI))) -> cli(XMI).
do(Step, Next) ->
    {cli,
        fun(Env) ->
            {Env@1, Result} = (erlang:element(2, Step))(Env),
...
-spec in(fun(() -> XML)) -> cli(XML).
in(Value) ->
    {cli, fun(Env) -> {Env, {ok, Value()}} end}.

-spec map(cli(XMN), fun((XMN) -> XMP)) -> cli(XMP).
map(Step, Next) ->
    {cli,
        fun(Env) ->
            {Env@1, Result} = (erlang:element(2, Step))(Env),
...
-spec 'try'(
    {ok, XMR} | {error, lustre_dev_tools@error:error()},
    fun((XMR) -> cli(XMU))
) -> cli(XMU).
'try'(Result, Next) ->
...
-spec log(binary(), fun(() -> cli(XMX))) -> cli(XMX).
log(Message, Next) ->
    {cli,
        fun(Env) ->
...
-spec success(binary(), fun(() -> cli(XNA))) -> cli(XNA).
...
-spec notify(binary(), fun(() -> cli(XND))) -> cli(XND).
notify(Message, Next) ->
    {cli,
        fun(Env) ->
            Env@1 = erlang:setelement(3, Env, case erlang:element(3, Env) of
...
-spec template(binary(), fun((binary()) -> cli(XNI))) -> cli(XNI).
template(Name, Next) ->
    {cli,
        fun(Env) ->
...
-spec get_config_value(
    ...
    XNO,
    list(binary()),
    fun((gleam@dict:dict(binary(), tom:toml()), list(binary())) -> {ok, XNO} |
        {error, any()}),
    fun((glint:flags()) -> {ok, XNO} | {error, any()})
) -> cli(XNO).
