...
-opaque task(KXH) :: {task,
        gleam@erlang@process:pid_(),
        gleam@erlang@process:pid_(),
        gleam@erlang@process:process_monitor(),
        gleam@erlang@process:selector(message(KXH))}.

-type await_error() :: timeout | {exit, gleam@dynamic:dynamic_()}.

-type message(KXI) :: {from_monitor, gleam@erlang@process:process_down()} |
    {from_subject, KXI}.

-spec async(fun(() -> KXJ)) -> task(KXJ).
async(Work) ->
    Owner = erlang:self(),
...
-spec try_await(task(KXN), integer()) -> {ok, KXN} | {error, await_error()}.
...
-spec await(task(KXR), integer()) -> KXR.
await(Task, Timeout) ->
...
-spec try_await_forever(task(KXT)) -> {ok, KXT} | {error, await_error()}.
...
-spec await_forever(task(KXX)) -> KXX.
await_forever(Task) ->
