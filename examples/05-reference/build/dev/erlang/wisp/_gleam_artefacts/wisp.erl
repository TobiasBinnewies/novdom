...
-spec method_override(gleam@http@request:request(WCJ)) -> gleam@http@request:request(WCJ).
...
-spec fn_with_bad_request_error(fun((WDK) -> {ok, WDL} | {error, any()})) -> fun((WDK) -> {ok,
        WDL} |
    {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((WDE, bitstring()) -> {ok, WDE} |
        {error, gleam@http@response:response(body())}),
    WDE
) -> {ok, {gleam@option:option(buffered_reader()), integer(), WDE}} |
    {error, gleam@http@response:response(body())}.
...
-spec sort_keys(list({binary(), WEC})) -> list({binary(), WEC}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_400(
    {ok, WEF} | {error, any()},
    fun((WEF) -> gleam@http@response:response(body()))
) -> gleam@http@response:response(body()).
or_400(Result, Next) ->
    case Result of
        {ok, Value} ->
...
-spec or_500({ok, WCW} | {error, any()}) -> {ok, WCW} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
