...
-opaque glint(LZS) :: {glint, config(), command_node(LZS)}.

-type args_count() :: {eq_args, integer()} | {min_args, integer()}.

-opaque command(LZT) :: {command,
        fun((named_args(), list(binary()), flags()) -> LZT),
        flags(),
        binary(),
        gleam@option:option(args_count()),
        list(binary())}.

-type internal_command(LZU) :: {internal_command,
        fun((named_args(), list(binary()), flags()) -> LZU),
        flags(),
        gleam@option:option(args_count()),
        list(binary())}.
...
-type command_node(LZV) :: {command_node,
        gleam@option:option(internal_command(LZV)),
        gleam@dict:dict(binary(), command_node(LZV)),
        flags(),
        binary()}.

-type out(LZW) :: {out, LZW} | {help, binary()}.
...
-opaque flag(LZX) :: {flag,
        binary(),
        binary(),
        fun((binary()) -> {ok, LZX} | {error, snag:snag()}),
        fun((flag_internals(LZX)) -> value()),
        fun((flags(), binary()) -> {ok, LZX} | {error, snag:snag()}),
        gleam@option:option(LZX)}.

-type flag_internals(LZY) :: {flag_internals,
        gleam@option:option(LZY),
        fun((binary()) -> {ok, LZY} | {error, snag:snag()})}.

-type flag_entry() :: {flag_entry, value(), binary()}.

-opaque flags() :: {flags, gleam@dict:dict(binary(), flag_entry())}.

-spec config(glint(MAF), config()) -> glint(MAF).
config(Glint, Config) ->
    erlang:setelement(2, Glint, Config).

-spec pretty_help(glint(MAI), pretty_help()) -> glint(MAI).
...
-spec as_module(glint(MAR)) -> glint(MAR).
as_module(Glint) ->
    config(Glint, erlang:setelement(4, erlang:element(2, Glint), true)).

-spec with_indent_width(glint(MAU), integer()) -> glint(MAU).
...
-spec with_max_output_width(glint(MAX), integer()) -> glint(MAX).
...
-spec with_min_first_column_width(glint(MBA), integer()) -> glint(MBA).
...
-spec with_column_gap(glint(MBD), integer()) -> glint(MBD).
...
-spec global_help(glint(MBM), binary()) -> glint(MBM).
...
-spec command_help(binary(), fun(() -> command(MCB))) -> command(MCB).
command_help(Desc, F) ->
    erlang:setelement(4, F(), Desc).

-spec unnamed_args(args_count(), fun(() -> command(MCE))) -> command(MCE).
unnamed_args(Count, F) ->
    erlang:setelement(5, F(), {some, Count}).

-spec named_arg(
    binary(),
    fun((fun((named_args()) -> binary())) -> command(MCH))
) -> command(MCH).
named_arg(Name, F) ->
    Cmd = (F(
        fun(Named_args) ->
...
-spec new_builder(
    binary(),
    fun((flag_internals(MEW)) -> value()),
    fun((flags(), binary()) -> {ok, MEW} | {error, snag:snag()}),
    fun((binary()) -> {ok, MEW} | {error, snag:snag()})
) -> flag(MEW).
...
-spec attempt(
    {ok, MFO} | {error, MFP},
    fun((MFO) -> {ok, any()} | {error, MFP})
) -> {ok, MFO} | {error, MFP}.
attempt(Val, F) ->
    gleam@result:'try'(Val, fun(A) -> gleam@result:replace(F(A), A) end).

-spec wrap_with_constraint(
    fun((binary()) -> {ok, MFI} | {error, snag:snag()}),
    fun((MFI) -> {ok, MFI} | {error, snag:snag()})
) -> fun((binary()) -> {ok, MFI} | {error, snag:snag()}).
wrap_with_constraint(P, Constraint) ->
    fun(Input) -> attempt(P(Input), Constraint) end.

-spec flag_constraint(flag(MFE), fun((MFE) -> {ok, MFE} | {error, snag:snag()})) -> flag(MFE).
...
-spec flag_help(flag(MFX), binary()) -> flag(MFX).
flag_help(Flag, Description) ->
    erlang:setelement(3, Flag, Description).

-spec flag_default(flag(MGA), MGA) -> flag(MGA).
...
-spec flag(
    flag(MCK),
    fun((fun((flags()) -> {ok, MCK} | {error, snag:snag()})) -> command(MCN))
) -> command(MCN).
flag(Flag, F) ->
    Cmd = F(
        fun(_capture) ->
...
-spec fold(flags(), MGD, fun((MGD, binary(), flag_entry()) -> MGD)) -> MGD.
fold(Flags, Acc, F) ->
...
-spec command(fun((named_args(), list(binary()), flags()) -> MBY)) -> command(MBY).
command(Runner) ->
...
-spec get_value(
    flags(),
    binary(),
    fun((flag_entry()) -> {ok, MGQ} | {error, snag:snag()})
) -> {ok, MGQ} | {error, snag:snag()}.
...
-spec get_flag(flags(), flag(MGT)) -> {ok, MGT} | {error, snag:snag()}.
get_flag(Flags, Flag) ->
...
-spec do_update_at(
    command_node(MHM),
    list(binary()),
    fun((command_node(MHM)) -> command_node(MHM))
) -> command_node(MHM).
...
-spec update_at(
    glint(MHG),
    list(binary()),
    fun((command_node(MHG)) -> command_node(MHG))
) -> glint(MHG).
...
-spec path_help(glint(MBI), list(binary()), binary()) -> glint(MBI).
...
-spec add(glint(MBP), list(binary()), command(MBP)) -> glint(MBP).
...
-spec group_flag(glint(MCQ), list(binary()), flag(any())) -> glint(MCQ).
...
-spec execute(glint(MCW), list(binary())) -> {ok, out(MCW)} | {error, binary()}.
execute(Glint, Args) ->
    Help_flag = <<"--"/utf8,
        (erlang:element(
            2,
...
-spec run_and_handle(glint(MDW), list(binary()), fun((MDW) -> any())) -> nil.
