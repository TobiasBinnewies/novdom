...
-spec decode_bits(
    bitstring(),
    fun((gleam@dynamic:dynamic_()) -> {ok, HMM} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, HMM} | {error, decode_error()}.
...
-spec do_decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, HMI} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, HMI} | {error, decode_error()}.
...
-spec decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, HME} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, HME} | {error, decode_error()}.
decode(Json, Decoder) ->
    do_decode(Json, Decoder).
...
-spec nullable(gleam@option:option(HMS), fun((HMS) -> json())) -> json().
...
-spec array(list(HMW), fun((HMW) -> json())) -> json().
