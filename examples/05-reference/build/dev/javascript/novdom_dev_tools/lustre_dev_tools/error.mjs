...
export class DependencyInstallationError extends $CustomType {
  constructor(reason, package_manager) {
    super();
    this.reason = reason;
...
export class DependencyNotFound extends $CustomType {
  constructor(package$) {
    super();
...
export class DependencyFaulty extends $CustomType {
  constructor(package$) {
    super();
    this.package = package$;
  }
...
function dependency_installation_error(reason, pm) {
  let message = "\nI ran into an unexpected issue while trying to install dependencies using {pm}.\nHere's the error message I got:\n\n    {reason}\n\nIf you think this is a bug, please open an issue at\nhttps://github.com/lustre-labs/dev-tools/issues/new with some details about what\nyou were trying to do when you ran into this issue.\n";
  let _pipe = message;
  let _pipe$1 = $string.replace(_pipe, "{reason}", reason);
  return $string.replace(_pipe$1, "{pm}", pm);
}

function dependency_not_found(package$) {
  ...
  let _pipe = message;
  return $string.replace(_pipe, "{package}", package$);
}

function dependency_faulty(package$) {
...
export function explain_to_string(error) {
  ...
  } else if (error instanceof InvalidTailwindBinary) {
    return invalid_tailwind_binary();
  } else if (error instanceof DependencyInstallationError) {
    let reason = error.reason;
    let pm = error.package_manager;
    return dependency_installation_error(reason, pm);
  } else if (error instanceof DependencyNotFound) {
    let package$ = error.package;
    return dependency_not_found(package$);
  } else {
    let package$ = error.package;
    return dependency_faulty(package$);
  }
}

export function explain(error) {
  let _pipe = error;
  let _pipe$1 = explain_to_string(_pipe);
  return $io.print_error(_pipe$1);
}
