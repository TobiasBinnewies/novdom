...
-export([esbuild_os/0, esbuild_cpu/0, tailwind_os/0, tailwind_cpu/0, prod/0, tailwind_entry/0, outdir/0, ext/0, detect_tailwind/0, port/0, proxy_from/0, proxy_to/0, package_manager/0]).
...
-spec prod() -> glint:flag(boolean()).
prod() ->
    Description = <<"Build for production. Minify the output, renaming variables and removing whitespace."/utf8>>,
    _pipe = glint:bool_flag(<<"prod"/utf8>>),
...
-spec package_manager() -> glint:flag(binary()).
package_manager() ->
    Description = <<"Use a package manager other than the default (bun) for the project."/utf8>>,
    Allowed = [<<"bun"/utf8>>, <<"npm"/utf8>>, <<"yarn"/utf8>>, <<"pnpm"/utf8>>],
    _pipe = glint:string_flag(<<"pm"/utf8>>),
    _pipe@1 = glint:flag_help(_pipe, Description),
    glint:flag_constraint(_pipe@1, glint@constraint:one_of(Allowed)).
