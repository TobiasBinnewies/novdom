...
-spec method_override(gleam@http@request:request(QYH)) -> gleam@http@request:request(QYH).
...
-spec fn_with_bad_request_error(fun((QZI) -> {ok, QZJ} | {error, any()})) -> fun((QZI) -> {ok,
        QZJ} |
    {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((QZC, bitstring()) -> {ok, QZC} |
        {error, gleam@http@response:response(body())}),
    QZC
) -> {ok, {gleam@option:option(buffered_reader()), integer(), QZC}} |
    {error, gleam@http@response:response(body())}.
...
-spec sort_keys(list({binary(), RAA})) -> list({binary(), RAA}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_400(
    {ok, RAD} | {error, any()},
    fun((RAD) -> gleam@http@response:response(body()))
) -> gleam@http@response:response(body()).
or_400(Result, Next) ->
    case Result of
        {ok, Value} ->
...
serve_static(Req, Prefix, Directory, Handler) ->
    ...
    case {erlang:element(2, Req), gleam@string:starts_with(Path, Prefix@1)} of
        {get, true} ->
            ...
            case simplifile:is_file(Path@1) of
                {ok, true} ->
...
-spec or_500({ok, QYU} | {error, any()}) -> {ok, QYU} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
