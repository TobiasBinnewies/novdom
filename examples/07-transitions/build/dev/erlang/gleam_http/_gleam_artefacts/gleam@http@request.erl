...
-spec set_header(request(JCB), binary(), binary()) -> request(JCB).
...
-spec set_query(request(JCX), list({binary(), binary()})) -> request(JCX).
set_query(Req, Query) ->
    Pair = fun(T) ->
...
-spec set_method(request(JDB), gleam@http:method()) -> request(JDB).
...
-spec set_scheme(request(JDI), gleam@http:scheme()) -> request(JDI).
set_scheme(Req, Scheme) ->
    erlang:setelement(5, Req, Scheme).

-spec set_host(request(JDL), binary()) -> request(JDL).
set_host(Req, Host) ->
    erlang:setelement(6, Req, Host).

-spec set_port(request(JDO), integer()) -> request(JDO).
set_port(Req, Port) ->
    erlang:setelement(7, Req, {some, Port}).

-spec set_path(request(JDR), binary()) -> request(JDR).
set_path(Req, Path) ->
    erlang:setelement(8, Req, Path).

-spec set_cookie(request(JDU), binary(), binary()) -> request(JDU).
