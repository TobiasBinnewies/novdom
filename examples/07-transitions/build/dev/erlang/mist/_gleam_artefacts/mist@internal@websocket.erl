...
-type valid_message(QPT) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, QPT}.

-type websocket_message(QPU) :: {valid, valid_message(QPU)} | invalid.
...
-type handler_message(QPV) :: {internal, gramps@websocket:frame()} | {user, QPV}.

-type websocket_state(QPW) :: {websocket_state,
        bitstring(),
        QPW,
...
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(QRE))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(QRE))).
...
-spec apply_frames(
    ...
    fun((QQU, websocket_connection(), handler_message(QQV)) -> gleam@otp@actor:next(QQV, QQU)),
    websocket_connection(),
    gleam@otp@actor:next(websocket_message(QQV), QQU),
    fun((QQU) -> nil)
) -> gleam@otp@actor:next(websocket_message(QQV), QQU).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {QQF,
        gleam@option:option(gleam@erlang@process:selector(QQG))}),
    fun((QQF) -> nil),
    fun((QQF, websocket_connection(), handler_message(QQG)) -> gleam@otp@actor:next(QQG, QQF)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok, gleam@erlang@process:subject(websocket_message(QQG))} | {error, nil}.
