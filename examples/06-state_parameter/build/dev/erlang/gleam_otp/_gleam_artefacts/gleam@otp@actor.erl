...
-type message(JTE) :: {message, JTE} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-type next(JTF, JTG) :: {continue,
        JTG,
        gleam@option:option(gleam@erlang@process:selector(JTF))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type init_result(JTH, JTI) :: {ready, JTH, gleam@erlang@process:selector(JTI)} |
    {failed, binary()}.

-type self(JTJ, JTK) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        JTJ,
        gleam@erlang@process:subject(JTK),
        gleam@erlang@process:selector(message(JTK)),
        gleam@otp@system:debug_state(),
        fun((JTK, JTJ) -> next(JTK, JTJ))}.

-type spec(JTL, JTM) :: {spec,
        fun(() -> init_result(JTL, JTM)),
        integer(),
        fun((JTM, JTL) -> next(JTM, JTL))}.

-type start_error() :: init_timeout |
    {init_failed, gleam@erlang@process:exit_reason()} |
    {init_crashed, gleam@dynamic:dynamic_()}.

-type start_init_message(JTN) :: {ack,
        {ok, gleam@erlang@process:subject(JTN)} |
            {error, gleam@erlang@process:exit_reason()}} |
    {mon, gleam@erlang@process:process_down()}.

-spec continue(JTU) -> next(any(), JTU).
continue(State) ->
    {continue, State, none}.

-spec with_selector(next(JTY, JTZ), gleam@erlang@process:selector(JTY)) -> next(JTY, JTZ).
...
-spec selecting_system_messages(gleam@erlang@process:selector(message(JUK))) -> gleam@erlang@process:selector(message(JUK)).
...
-spec receive_message(self(any(), JUG)) -> message(JUG).
...
-spec init_selector(
    gleam@erlang@process:subject(JYY),
    gleam@erlang@process:selector(JYY)
) -> gleam@erlang@process:selector(message(JYY)).
...
-spec initialise_actor(
    spec(any(), JVB),
    gleam@erlang@process:subject({ok, gleam@erlang@process:subject(JVB)} |
        {error, gleam@erlang@process:exit_reason()})
...
-spec start_spec(spec(any(), JVO)) -> {ok, gleam@erlang@process:subject(JVO)} |
    {error, start_error()}.
start_spec(Spec) ->
...
-spec start(JVU, fun((JVV, JVU) -> next(JVV, JVU))) -> {ok,
        gleam@erlang@process:subject(JVV)} |
    {error, start_error()}.
start(State, Loop) ->
    start_spec(
        {spec,
...
-spec send(gleam@erlang@process:subject(JWB), JWB) -> nil.
send(Subject, Msg) ->
    gleam@erlang@process:send(Subject, Msg).

-spec call(
    gleam@erlang@process:subject(JWD),
    fun((gleam@erlang@process:subject(JWF)) -> JWD),
    integer()
) -> JWF.
