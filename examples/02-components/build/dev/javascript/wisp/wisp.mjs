...
class ReadingFinished extends $CustomType {}
...
function buffered_read(reader, chunk_size) {
  ...
  if ($.length == 0) {
    ...
    return new Ok(new Chunk(reader.buffer, reader.reader));
  }
}
...
function read_body_loop(reader, read_chunk_size, max_body_size, accumulator) {
  return $result.try$(
    ...
    (chunk) => {
      if (chunk instanceof ReadingFinished) {
...
function remove_preceeding_slashes(loop$string) {
  while (true) {
...
export function serve_static(req, prefix, directory, handler) {
  ...
  if ($ instanceof $http.Get && $1) {
    let path$1 = (() => {
      ...
      return ((_capture) => { return join_path(directory, _capture); })(_pipe$2);
    })();
    let mime_type = (() => {
      let _pipe = req.path;
...
function make_connection(body_reader, secret_key_base) {
...
export function create_canned_connection(body, secret_key_base) {
  return make_connection(
    (_) => {
      return new Ok(
        new Chunk(body, (_) => { return new Ok(new ReadingFinished()); }),
      );
    },
    secret_key_base,
  );
