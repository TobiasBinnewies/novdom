...
export function prod() {
  let description = "Build for production. Minify the output, renaming variables and removing whitespace.";
  let _pipe = $glint.bool_flag("prod");
...
export function package_manager() {
  let description = "Use a package manager other than the default (bun) for the project.";
  let allowed = toList(["bun", "npm", "yarn", "pnpm"]);
  let _pipe = $glint.string_flag("pm");
  let _pipe$1 = $glint.flag_help(_pipe, description);
  return $glint.flag_constraint(_pipe$1, $constraint.one_of(allowed));
}
