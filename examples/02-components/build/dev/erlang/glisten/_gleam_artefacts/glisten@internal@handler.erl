...
-type message(NPH) :: {internal, internal_message()} | {user, NPH}.

-type loop_message(NPI) :: {packet, bitstring()} | {custom, NPI}.

-type loop_state(NPJ, NPK) :: {loop_state,
        {ok, {{integer(), integer(), integer(), integer()}, integer()}} |
            {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(NPJ)),
        glisten@transport:transport(),
        NPK}.

-type connection(NPL) :: {connection,
        {ok, {{integer(), integer(), integer(), integer()}, integer()}} |
            {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(NPL))}.

-type handler(NPM, NPN) :: {handler,
        glisten@socket:socket(),
        fun((loop_message(NPM), NPN, connection(NPM)) -> gleam@otp@actor:next(loop_message(NPM), NPN)),
        fun((connection(NPM)) -> {NPN,
            gleam@option:option(gleam@erlang@process:selector(NPM))}),
        gleam@option:option(fun((NPN) -> nil)),
        glisten@transport:transport()}.

-spec start(handler(NPX, any())) -> {ok,
        gleam@erlang@process:subject(message(NPX))} |
