...
-spec set_header(request(JHN), binary(), binary()) -> request(JHN).
...
-spec set_query(request(JIJ), list({binary(), binary()})) -> request(JIJ).
set_query(Req, Query) ->
    Pair = fun(T) ->
...
-spec set_method(request(JIN), gleam@http:method()) -> request(JIN).
...
-spec set_scheme(request(JIU), gleam@http:scheme()) -> request(JIU).
set_scheme(Req, Scheme) ->
    erlang:setelement(5, Req, Scheme).

-spec set_host(request(JIX), binary()) -> request(JIX).
set_host(Req, Host) ->
    erlang:setelement(6, Req, Host).

-spec set_port(request(JJA), integer()) -> request(JJA).
set_port(Req, Port) ->
    erlang:setelement(7, Req, {some, Port}).

-spec set_path(request(JJD), binary()) -> request(JJD).
set_path(Req, Path) ->
    erlang:setelement(8, Req, Path).

-spec set_cookie(request(JJG), binary(), binary()) -> request(JJG).
