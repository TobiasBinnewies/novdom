...
-spec map_response_body(
    fun((JSU) -> gleam@http@response:response(JSG)),
    fun((JSG) -> JSH)
) -> fun((JSU) -> gleam@http@response:response(JSH)).
...
-spec prepend_response_header(
    fun((JSY) -> gleam@http@response:response(JTA)),
    binary(),
    binary()
) -> fun((JSY) -> gleam@http@response:response(JTA)).
...
-spec ensure_post(gleam@http@request:request(JSL)) -> {ok,
        gleam@http@request:request(JSL)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(JUH)) -> JUP)) -> fun((gleam@http@request:request(JUH)) -> JUP).
