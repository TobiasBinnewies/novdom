...
-type response(JOH) :: {response, integer(), list({binary(), binary()}), JOH}.
...
-spec set_header(response(JOW), binary(), binary()) -> response(JOW).
...
-spec prepend_header(response(JOZ), binary(), binary()) -> response(JOZ).
prepend_header(Response, Key, Value) ->
    Headers = [{gleam@string:lowercase(Key), Value} |
        erlang:element(3, Response)],
    erlang:setelement(3, Response, Headers).

-spec set_body(response(any()), JPE) -> response(JPE).
set_body(Response, Body) ->
    {response, Status, Headers, _} = Response,
    {response, Status, Headers, Body}.

-spec try_map(response(JOI), fun((JOI) -> {ok, JOK} | {error, JOL})) -> {ok,
        response(JOK)} |
    {error, JOL}.
...
-spec map(response(JPG), fun((JPG) -> JPI)) -> response(JPI).
...
-spec expire_cookie(response(JPQ), binary(), gleam@http@cookie:attributes()) -> response(JPQ).
