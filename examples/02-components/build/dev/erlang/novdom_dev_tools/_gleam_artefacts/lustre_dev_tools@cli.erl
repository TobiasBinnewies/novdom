...
-opaque cli(BRO) :: {cli,
        fun((env()) -> {env(),
            {ok, BRO} | {error, lustre_dev_tools@error:error()}})}.
...
-spec run(cli(BRP), glint:flags()) -> {ok, BRP} |
...
-spec from_result({ok, BRX} | {error, lustre_dev_tools@error:error()}) -> cli(BRX).
from_result(Result) ->
    {cli, fun(Env) -> {Env, Result} end}.

-spec do(cli(BSB), fun((BSB) -> cli(BSD))) -> cli(BSD).
do(Step, Next) ->
    {cli,
        fun(Env) ->
            {Env@1, Result} = (erlang:element(2, Step))(Env),
...
-spec in(fun(() -> BSG)) -> cli(BSG).
in(Value) ->
    {cli, fun(Env) -> {Env, {ok, Value()}} end}.

-spec map(cli(BSI), fun((BSI) -> BSK)) -> cli(BSK).
map(Step, Next) ->
    {cli,
        fun(Env) ->
            {Env@1, Result} = (erlang:element(2, Step))(Env),
...
-spec 'try'(
    {ok, BSM} | {error, lustre_dev_tools@error:error()},
    fun((BSM) -> cli(BSP))
) -> cli(BSP).
'try'(Result, Next) ->
...
-spec log(binary(), fun(() -> cli(BSS))) -> cli(BSS).
log(Message, Next) ->
    {cli,
        fun(Env) ->
...
-spec success(binary(), fun(() -> cli(BSV))) -> cli(BSV).
...
-spec notify(binary(), fun(() -> cli(BSY))) -> cli(BSY).
notify(Message, Next) ->
    {cli,
        fun(Env) ->
            Env@1 = erlang:setelement(3, Env, case erlang:element(3, Env) of
...
-spec template(binary(), fun((binary()) -> cli(BTD))) -> cli(BTD).
template(Name, Next) ->
    {cli,
        fun(Env) ->
...
-spec get_config_value(
    ...
    BTJ,
    list(binary()),
    fun((gleam@dict:dict(binary(), tom:toml()), list(binary())) -> {ok, BTJ} |
        {error, any()}),
    fun((glint:flags()) -> {ok, BTJ} | {error, any()})
) -> cli(BTJ).
