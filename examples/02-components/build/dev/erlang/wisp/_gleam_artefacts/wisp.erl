...
-spec method_override(gleam@http@request:request(WCI)) -> gleam@http@request:request(WCI).
...
-spec fn_with_bad_request_error(fun((WDJ) -> {ok, WDK} | {error, any()})) -> fun((WDJ) -> {ok,
        WDK} |
    {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((WDD, bitstring()) -> {ok, WDD} |
        {error, gleam@http@response:response(body())}),
    WDD
) -> {ok, {gleam@option:option(buffered_reader()), integer(), WDD}} |
    {error, gleam@http@response:response(body())}.
...
-spec sort_keys(list({binary(), WEB})) -> list({binary(), WEB}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_400(
    {ok, WEE} | {error, any()},
    fun((WEE) -> gleam@http@response:response(body()))
) -> gleam@http@response:response(body()).
or_400(Result, Next) ->
    case Result of
        {ok, Value} ->
...
-spec or_500({ok, WCV} | {error, any()}) -> {ok, WCV} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
