...
-type message(JYQ) :: {message, JYQ} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-type next(JYR, JYS) :: {continue,
        JYS,
        gleam@option:option(gleam@erlang@process:selector(JYR))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type init_result(JYT, JYU) :: {ready, JYT, gleam@erlang@process:selector(JYU)} |
    {failed, binary()}.

-type self(JYV, JYW) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        JYV,
        gleam@erlang@process:subject(JYW),
        gleam@erlang@process:selector(message(JYW)),
        gleam@otp@system:debug_state(),
        fun((JYW, JYV) -> next(JYW, JYV))}.

-type spec(JYX, JYY) :: {spec,
        fun(() -> init_result(JYX, JYY)),
        integer(),
        fun((JYY, JYX) -> next(JYY, JYX))}.

-type start_error() :: init_timeout |
    {init_failed, gleam@erlang@process:exit_reason()} |
    {init_crashed, gleam@dynamic:dynamic_()}.

-type start_init_message(JYZ) :: {ack,
        {ok, gleam@erlang@process:subject(JYZ)} |
            {error, gleam@erlang@process:exit_reason()}} |
    {mon, gleam@erlang@process:process_down()}.

-spec continue(JZG) -> next(any(), JZG).
continue(State) ->
    {continue, State, none}.

-spec with_selector(next(JZK, JZL), gleam@erlang@process:selector(JZK)) -> next(JZK, JZL).
...
-spec selecting_system_messages(gleam@erlang@process:selector(message(JZW))) -> gleam@erlang@process:selector(message(JZW)).
...
-spec receive_message(self(any(), JZS)) -> message(JZS).
...
-spec init_selector(
    gleam@erlang@process:subject(KEK),
    gleam@erlang@process:selector(KEK)
) -> gleam@erlang@process:selector(message(KEK)).
...
-spec initialise_actor(
    spec(any(), KAN),
    gleam@erlang@process:subject({ok, gleam@erlang@process:subject(KAN)} |
        {error, gleam@erlang@process:exit_reason()})
...
-spec start_spec(spec(any(), KBA)) -> {ok, gleam@erlang@process:subject(KBA)} |
    {error, start_error()}.
start_spec(Spec) ->
...
-spec start(KBG, fun((KBH, KBG) -> next(KBH, KBG))) -> {ok,
        gleam@erlang@process:subject(KBH)} |
    {error, start_error()}.
start(State, Loop) ->
    start_spec(
        {spec,
...
-spec send(gleam@erlang@process:subject(KBN), KBN) -> nil.
send(Subject, Msg) ->
    gleam@erlang@process:send(Subject, Msg).

-spec call(
    gleam@erlang@process:subject(KBP),
    fun((gleam@erlang@process:subject(KBR)) -> KBP),
    integer()
) -> KBR.
