...
-type spec(KKL, KKM) :: {spec,
        KKL,
        integer(),
        integer(),
        fun((children(KKL)) -> children(KKM))}.

-opaque children(KKN) :: {ready, starter(KKN)} | {failed, child_start_error()}.

-opaque child_spec(KKO, KKP, KKQ) :: {child_spec,
        fun((KKP) -> {ok, gleam@erlang@process:subject(KKO)} |
            {error, gleam@otp@actor:start_error()}),
        fun((KKP, gleam@erlang@process:subject(KKO)) -> KKQ)}.
...
-spec start_child(child_spec(any(), KKX, KKY), KKX) -> {ok, child(KKY)} |
    {error, child_start_error()}.
...
-spec perform_instruction_for_child(
    KLL,
    instruction(),
    child_spec(any(), KLL, KLN),
    child(KLN)
) -> {ok, {child(KLN), instruction()}} | {error, child_start_error()}.
...
-spec start_and_add_child(starter(KME), child_spec(any(), KME, KMH)) -> children(KMH).
...
-spec add(children(KMM), child_spec(any(), KMM, KMP)) -> children(KMP).
...
-spec supervisor(
    fun((KMU) -> {ok, gleam@erlang@process:subject(KMV)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(KMV, KMU, KMU).
supervisor(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-spec worker(
    fun((KNC) -> {ok, gleam@erlang@process:subject(KND)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(KND, KNC, KNC).
worker(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-spec returning(
    child_spec(KNK, KNL, any()),
    fun((KNL, gleam@erlang@process:subject(KNK)) -> KNR)
) -> child_spec(KNK, KNL, KNR).
returning(Child, Updater) ->
    {child_spec, erlang:element(2, Child), Updater}.

-spec init(spec(any(), KNW)) -> gleam@otp@actor:init_result(state(KNW), message()).
init(Spec) ->
...
-spec handle_exit(gleam@erlang@process:pid_(), state(KOC)) -> gleam@otp@actor:next(message(), state(KOC)).
...
-spec loop(message(), state(KOH)) -> gleam@otp@actor:next(message(), state(KOH)).
loop(Message, State) ->
    case Message of
        {exit, Exit_message} ->
