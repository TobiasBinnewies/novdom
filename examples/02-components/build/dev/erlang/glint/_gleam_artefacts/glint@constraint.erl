...
-spec one_of(list(LSY)) -> fun((LSY) -> {ok, LSY} | {error, snag:snag()}).
one_of(Allowed) ->
...
-spec none_of(list(LTB)) -> fun((LTB) -> {ok, LTB} | {error, snag:snag()}).
none_of(Disallowed) ->
...
-spec each(fun((LTE) -> {ok, LTE} | {error, snag:snag()})) -> fun((list(LTE)) -> {ok,
        list(LTE)} |
    {error, snag:snag()}).
each(Constraint) ->
