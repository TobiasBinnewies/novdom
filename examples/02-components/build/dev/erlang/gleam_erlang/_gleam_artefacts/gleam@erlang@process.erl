...
-opaque subject(IGH) :: {subject, pid_(), gleam@erlang:reference_()} |
    {gleam_phantom, IGH}.

-type do_not_leak() :: any().

-type selector(IGI) :: any() | {gleam_phantom, IGI}.
...
-type call_error(IGJ) :: {callee_down, gleam@dynamic:dynamic_()} |
    call_timeout |
    {gleam_phantom, IGJ}.
...
-spec select(selector(IHA), integer()) -> {ok, IHA} | {error, nil}.
select(From, Within) ->
    gleam_erlang_ffi:select(From, Within).

-spec select_forever(selector(IHE)) -> IHE.
select_forever(From) ->
    gleam_erlang_ffi:select(From).

-spec map_selector(selector(IHG), fun((IHG) -> IHI)) -> selector(IHI).
map_selector(A, B) ->
    gleam_erlang_ffi:map_selector(A, B).

-spec merge_selector(selector(IHK), selector(IHK)) -> selector(IHK).
merge_selector(A, B) ->
...
-spec selecting_trapped_exits(selector(IHO), fun((exit_message()) -> IHO)) -> selector(IHO).
...
-spec selecting(selector(IHR), subject(IHT), fun((IHT) -> IHR)) -> selector(IHR).
...
-spec 'receive'(subject(IGU), integer()) -> {ok, IGU} | {error, nil}.
...
-spec selecting_record2(
    selector(IHW),
    any(),
    fun((gleam@dynamic:dynamic_()) -> IHW)
) -> selector(IHW).
...
-spec selecting_record3(
    selector(IIA),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> IIA)
) -> selector(IIA).
...
-spec selecting_record4(
    selector(IIE),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> IIE)
) -> selector(IIE).
...
-spec selecting_record5(
    selector(III),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> III)
) -> selector(III).
...
-spec selecting_record6(
    selector(IIM),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> IIM)
) -> selector(IIM).
...
-spec selecting_record7(
    selector(IIQ),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> IIQ)
) -> selector(IIQ).
...
-spec selecting_record8(
    selector(IIU),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> IIU)
) -> selector(IIU).
...
-spec selecting_anything(selector(IIY), fun((gleam@dynamic:dynamic_()) -> IIY)) -> selector(IIY).
...
-spec try_call(subject(IJJ), fun((subject(IJL)) -> IJJ), integer()) -> {ok, IJL} |
    {error, call_error(IJL)}.
...
-spec call(subject(IJQ), fun((subject(IJS)) -> IJQ), integer()) -> IJS.
...
-spec send_after(subject(IJV), integer(), IJV) -> timer().
