...
-spec push_key({ok, UNP} | {error, get_error()}, binary()) -> {ok, UNP} |
    {error, get_error()}.
push_key(Result, Key) ->
    case Result of
        {ok, T} ->
...
-spec expect_end_of_line(
    list(binary()),
    fun((list(binary())) -> {ok, {UPU, list(binary())}} | {error, parse_error()})
) -> {ok, {UPU, list(binary())}} | {error, parse_error()}.
...
-spec do(
    {ok, {UQF, list(binary())}} | {error, parse_error()},
    fun((UQF, list(binary())) -> {ok, UQI} | {error, parse_error()})
) -> {ok, UQI} | {error, parse_error()}.
do(Result, Next) ->
    case Result of
        {ok, {A, Input}} ->
            Next(A, Input);
...
-spec expect(
    list(binary()),
    binary(),
    fun((list(binary())) -> {ok, {UQN, list(binary())}} | {error, parse_error()})
) -> {ok, {UQN, list(binary())}} | {error, parse_error()}.
