...
-spec create(
    ...
    fun((FOB) -> FOB),
    fun((FOA, FOB) -> FOA),
    fun((FOA, FOA) -> gleam@order:order())
) -> fun((FOA, FOA, FOB) -> {ok, gleam@iterator:iterator(FOA)} | {error, nil}).
...
-spec create_infinite(
    fun((FOF) -> boolean()),
    fun((FOF, FOG) -> FOF),
    fun((FOF, FOF) -> gleam@order:order())
) -> fun((FOF, FOG) -> {ok, gleam@iterator:iterator(FOF)} | {error, nil}).
