...
-spec push_key({ok, SPW} | {error, get_error()}, binary()) -> {ok, SPW} |
    {error, get_error()}.
push_key(Result, Key) ->
    case Result of
        {ok, T} ->
...
-spec expect_end_of_line(
    list(binary()),
    fun((list(binary())) -> {ok, {SSB, list(binary())}} | {error, parse_error()})
) -> {ok, {SSB, list(binary())}} | {error, parse_error()}.
...
-spec do(
    {ok, {SSM, list(binary())}} | {error, parse_error()},
    fun((SSM, list(binary())) -> {ok, SSP} | {error, parse_error()})
) -> {ok, SSP} | {error, parse_error()}.
do(Result, Next) ->
    case Result of
        {ok, {A, Input}} ->
            Next(A, Input);
...
-spec expect(
    list(binary()),
    binary(),
    fun((list(binary())) -> {ok, {SSU, list(binary())}} | {error, parse_error()})
) -> {ok, {SSU, list(binary())}} | {error, parse_error()}.
