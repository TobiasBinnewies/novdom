...
-type message(NHU) :: {internal, internal_message()} | {user, NHU}.

-type loop_message(NHV) :: {packet, bitstring()} | {custom, NHV}.

-type loop_state(NHW, NHX) :: {loop_state,
        {ok, {{integer(), integer(), integer(), integer()}, integer()}} |
            {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(NHW)),
        glisten@transport:transport(),
        NHX}.

-type connection(NHY) :: {connection,
        {ok, {{integer(), integer(), integer(), integer()}, integer()}} |
            {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(NHY))}.

-type handler(NHZ, NIA) :: {handler,
        glisten@socket:socket(),
        fun((loop_message(NHZ), NIA, connection(NHZ)) -> gleam@otp@actor:next(loop_message(NHZ), NIA)),
        fun((connection(NHZ)) -> {NIA,
            gleam@option:option(gleam@erlang@process:selector(NHZ))}),
        gleam@option:option(fun((NIA) -> nil)),
        glisten@transport:transport()}.

-spec start(handler(NIK, any())) -> {ok,
        gleam@erlang@process:subject(message(NIK))} |
