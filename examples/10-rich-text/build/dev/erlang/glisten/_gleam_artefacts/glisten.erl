...
-type message(NVO) :: {packet, bitstring()} | {user, NVO}.

-type connection(NVP) :: {connection,
        {ok, {{integer(), integer(), integer(), integer()}, integer()}} |
            {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(NVP))}.

-opaque handler(NVQ, NVR) :: {handler,
        fun((connection(NVQ)) -> {NVR,
            gleam@option:option(gleam@erlang@process:selector(NVQ))}),
        fun((message(NVQ), NVR, connection(NVQ)) -> gleam@otp@actor:next(message(NVQ), NVR)),
        gleam@option:option(fun((NVR) -> nil)),
        integer()}.
...
-spec convert_on_init(
    fun((connection(NWO)) -> {NWQ,
        gleam@option:option(gleam@erlang@process:selector(NWO))})
) -> fun((glisten@internal@handler:connection(NWO)) -> {NWQ,
    gleam@option:option(gleam@erlang@process:selector(NWO))}).
...
-spec handler(
    fun((connection(NWW)) -> {NWY,
        gleam@option:option(gleam@erlang@process:selector(NWW))}),
    fun((message(NWW), NWY, connection(NWW)) -> gleam@otp@actor:next(message(NWW), NWY))
) -> handler(NWW, NWY).
handler(On_init, Loop) ->
    {handler, On_init, Loop, none, 10}.

-spec map_user_selector(gleam@erlang@process:selector(message(NWD))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(NWD)).
...
-spec convert_loop(
    fun((message(NWI), NWJ, connection(NWI)) -> gleam@otp@actor:next(message(NWI), NWJ))
) -> fun((glisten@internal@handler:loop_message(NWI), NWJ, glisten@internal@handler:connection(NWI)) -> gleam@otp@actor:next(glisten@internal@handler:loop_message(NWI), NWJ)).
convert_loop(Loop) ->
    fun(Msg, Data, Conn) ->
        Conn@1 = {connection,
            erlang:element(2, Conn),
...
-spec with_close(handler(NXF, NXG), fun((NXG) -> nil)) -> handler(NXF, NXG).
with_close(Handler, On_close) ->
    erlang:setelement(4, Handler, {some, On_close}).

-spec with_pool_size(handler(NXL, NXM), integer()) -> handler(NXL, NXM).
