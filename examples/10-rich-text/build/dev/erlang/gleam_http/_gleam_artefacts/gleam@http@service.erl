...
-spec map_response_body(
    fun((JNI) -> gleam@http@response:response(JMU)),
    fun((JMU) -> JMV)
) -> fun((JNI) -> gleam@http@response:response(JMV)).
...
-spec prepend_response_header(
    fun((JNM) -> gleam@http@response:response(JNO)),
    binary(),
    binary()
) -> fun((JNM) -> gleam@http@response:response(JNO)).
...
-spec ensure_post(gleam@http@request:request(JMZ)) -> {ok,
        gleam@http@request:request(JMZ)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(JOV)) -> JPD)) -> fun((gleam@http@request:request(JOV)) -> JPD).
