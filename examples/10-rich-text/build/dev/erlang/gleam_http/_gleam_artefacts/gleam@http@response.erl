...
-type response(JIV) :: {response, integer(), list({binary(), binary()}), JIV}.
...
-spec set_header(response(JJK), binary(), binary()) -> response(JJK).
...
-spec prepend_header(response(JJN), binary(), binary()) -> response(JJN).
prepend_header(Response, Key, Value) ->
    Headers = [{gleam@string:lowercase(Key), Value} |
        erlang:element(3, Response)],
    erlang:setelement(3, Response, Headers).

-spec set_body(response(any()), JJS) -> response(JJS).
set_body(Response, Body) ->
    {response, Status, Headers, _} = Response,
    {response, Status, Headers, Body}.

-spec try_map(response(JIW), fun((JIW) -> {ok, JIY} | {error, JIZ})) -> {ok,
        response(JIY)} |
    {error, JIZ}.
...
-spec map(response(JJU), fun((JJU) -> JJW)) -> response(JJW).
...
-spec expire_cookie(response(JKE), binary(), gleam@http@cookie:attributes()) -> response(JKE).
