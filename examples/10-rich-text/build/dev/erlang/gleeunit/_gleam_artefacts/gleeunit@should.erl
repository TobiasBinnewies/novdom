...
-spec equal(LJS, LJS) -> nil.
equal(A, B) ->
    gleeunit_ffi:should_equal(A, B).

-spec not_equal(LJT, LJT) -> nil.
not_equal(A, B) ->
    gleeunit_ffi:should_not_equal(A, B).

-spec be_ok({ok, LJU} | {error, any()}) -> LJU.
be_ok(A) ->
    gleeunit_ffi:should_be_ok(A).

-spec be_error({ok, any()} | {error, LJZ}) -> LJZ.
be_error(A) ->
    gleeunit_ffi:should_be_error(A).

-spec be_some(gleam@option:option(LKC)) -> LKC.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
