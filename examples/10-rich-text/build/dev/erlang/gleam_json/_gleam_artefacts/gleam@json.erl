...
-spec decode_bits(
    bitstring(),
    fun((gleam@dynamic:dynamic_()) -> {ok, HHA} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, HHA} | {error, decode_error()}.
...
-spec do_decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, HGW} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, HGW} | {error, decode_error()}.
...
-spec decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, HGS} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, HGS} | {error, decode_error()}.
decode(Json, Decoder) ->
    do_decode(Json, Decoder).
...
-spec nullable(gleam@option:option(HHG), fun((HHG) -> json())) -> json().
...
-spec array(list(HHK), fun((HHK) -> json())) -> json().
