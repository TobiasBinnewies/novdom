...
-opaque task(KRV) :: {task,
        gleam@erlang@process:pid_(),
        gleam@erlang@process:pid_(),
        gleam@erlang@process:process_monitor(),
        gleam@erlang@process:selector(message(KRV))}.

-type await_error() :: timeout | {exit, gleam@dynamic:dynamic_()}.

-type message(KRW) :: {from_monitor, gleam@erlang@process:process_down()} |
    {from_subject, KRW}.

-spec async(fun(() -> KRX)) -> task(KRX).
async(Work) ->
    Owner = erlang:self(),
...
-spec try_await(task(KSB), integer()) -> {ok, KSB} | {error, await_error()}.
...
-spec await(task(KSF), integer()) -> KSF.
await(Task, Timeout) ->
...
-spec try_await_forever(task(KSH)) -> {ok, KSH} | {error, await_error()}.
...
-spec await_forever(task(KSL)) -> KSL.
await_forever(Task) ->
