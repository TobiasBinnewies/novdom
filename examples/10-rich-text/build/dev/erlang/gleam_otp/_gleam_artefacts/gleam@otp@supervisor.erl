...
-type spec(KEZ, KFA) :: {spec,
        KEZ,
        integer(),
        integer(),
        fun((children(KEZ)) -> children(KFA))}.

-opaque children(KFB) :: {ready, starter(KFB)} | {failed, child_start_error()}.

-opaque child_spec(KFC, KFD, KFE) :: {child_spec,
        fun((KFD) -> {ok, gleam@erlang@process:subject(KFC)} |
            {error, gleam@otp@actor:start_error()}),
        fun((KFD, gleam@erlang@process:subject(KFC)) -> KFE)}.
...
-spec start_child(child_spec(any(), KFL, KFM), KFL) -> {ok, child(KFM)} |
    {error, child_start_error()}.
...
-spec perform_instruction_for_child(
    KFZ,
    instruction(),
    child_spec(any(), KFZ, KGB),
    child(KGB)
) -> {ok, {child(KGB), instruction()}} | {error, child_start_error()}.
...
-spec start_and_add_child(starter(KGS), child_spec(any(), KGS, KGV)) -> children(KGV).
...
-spec add(children(KHA), child_spec(any(), KHA, KHD)) -> children(KHD).
...
-spec supervisor(
    fun((KHI) -> {ok, gleam@erlang@process:subject(KHJ)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(KHJ, KHI, KHI).
supervisor(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-spec worker(
    fun((KHQ) -> {ok, gleam@erlang@process:subject(KHR)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(KHR, KHQ, KHQ).
worker(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-spec returning(
    child_spec(KHY, KHZ, any()),
    fun((KHZ, gleam@erlang@process:subject(KHY)) -> KIF)
) -> child_spec(KHY, KHZ, KIF).
returning(Child, Updater) ->
    {child_spec, erlang:element(2, Child), Updater}.

-spec init(spec(any(), KIK)) -> gleam@otp@actor:init_result(state(KIK), message()).
init(Spec) ->
...
-spec handle_exit(gleam@erlang@process:pid_(), state(KIQ)) -> gleam@otp@actor:next(message(), state(KIQ)).
...
-spec loop(message(), state(KIV)) -> gleam@otp@actor:next(message(), state(KIV)).
loop(Message, State) ->
    case Message of
        {exit, Exit_message} ->
