...
-opaque cli(WTE) :: {cli,
        fun((env()) -> {env(),
            {ok, WTE} | {error, lustre_dev_tools@error:error()}})}.
...
-spec run(cli(WTF), glint:flags()) -> {ok, WTF} |
...
-spec from_result({ok, WTN} | {error, lustre_dev_tools@error:error()}) -> cli(WTN).
from_result(Result) ->
    {cli, fun(Env) -> {Env, Result} end}.

-spec do(cli(WTR), fun((WTR) -> cli(WTT))) -> cli(WTT).
do(Step, Next) ->
    {cli,
        fun(Env) ->
            {Env@1, Result} = (erlang:element(2, Step))(Env),
...
-spec in(fun(() -> WTW)) -> cli(WTW).
in(Value) ->
    {cli, fun(Env) -> {Env, {ok, Value()}} end}.

-spec map(cli(WTY), fun((WTY) -> WUA)) -> cli(WUA).
map(Step, Next) ->
    {cli,
        fun(Env) ->
            {Env@1, Result} = (erlang:element(2, Step))(Env),
...
-spec 'try'(
    {ok, WUC} | {error, lustre_dev_tools@error:error()},
    fun((WUC) -> cli(WUF))
) -> cli(WUF).
'try'(Result, Next) ->
...
-spec log(binary(), fun(() -> cli(WUI))) -> cli(WUI).
log(Message, Next) ->
    {cli,
        fun(Env) ->
...
-spec success(binary(), fun(() -> cli(WUL))) -> cli(WUL).
...
-spec notify(binary(), fun(() -> cli(WUO))) -> cli(WUO).
notify(Message, Next) ->
    {cli,
        fun(Env) ->
            Env@1 = erlang:setelement(3, Env, case erlang:element(3, Env) of
...
-spec template(binary(), fun((binary()) -> cli(WUT))) -> cli(WUT).
template(Name, Next) ->
    {cli,
        fun(Env) ->
...
-spec get_config_value(
    ...
    WUZ,
    list(binary()),
    fun((gleam@dict:dict(binary(), tom:toml()), list(binary())) -> {ok, WUZ} |
        {error, any()}),
    fun((glint:flags()) -> {ok, WUZ} | {error, any()})
) -> cli(WUZ).
