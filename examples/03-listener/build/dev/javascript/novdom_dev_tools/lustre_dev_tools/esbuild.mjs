...
class PackageJson extends $CustomType {
  constructor(main, exports) {
    super();
    this.main = main;
...
function module_main_file(json) {
...
function get_module_main(name) {
  let root = $project.root();
  let module = $filepath.join(root, "node_modules/" + name);
  let module_json = $filepath.join(module, "package.json");
  let $ = $simplifile.is_directory(module);
  if ($.isOk() && !$[0]) {
    return new Error(new DependencyNotFound(name));
  } else if (!$.isOk()) {
    return new Error(new DependencyNotFound(name));
  } else {
    return $result.try$(
      (() => {
        let _pipe = $simplifile.read(module_json);
        return $result.replace_error(_pipe, new DependencyFaulty(name));
      })(),
      (json) => {
        return $result.try$(
          (() => {
            let _pipe = module_main_file(json);
            return $result.replace_error(_pipe, new DependencyFaulty(name));
          })(),
          (main_file) => {
            let $1 = $filepath.expand($filepath.join(module, main_file));
            if (!$1.isOk()) {
              throw makeError(
                "assignment_no_match",
