...
-export([invalid_proxy_target/1, explain_to_string/1, explain/1]).
-export_type([error/0]).
...
-type error() :: {build_error, binary()} |
    ...
    {dependency_installation_error, binary(), binary()} |
    {dependency_not_found, binary()} |
    {dependency_faulty, binary()}.
...
-spec dependency_installation_error(binary(), binary()) -> binary().
...
"/utf8>>,
    _pipe = Message,
    _pipe@1 = gleam@string:replace(_pipe, <<"{reason}"/utf8>>, Reason),
    gleam@string:replace(_pipe@1, <<"{pm}"/utf8>>, Pm).

-spec dependency_not_found(binary()) -> binary().
dependency_not_found(Package) ->
    Message = <<"
...
"/utf8>>,
    _pipe = Message,
    gleam@string:replace(_pipe, <<"{package}"/utf8>>, Package).

-spec dependency_faulty(binary()) -> binary().
dependency_faulty(Package) ->
    Message = <<"
...
-spec explain_to_string(error()) -> binary().
explain_to_string(Error) ->
    case Error of
        {build_error, Reason} ->
            build_error(Reason);
        ...
        {dependency_installation_error, Reason@9, Pm} ->
            dependency_installation_error(Reason@9, Pm);

        {dependency_not_found, Package} ->
            dependency_not_found(Package);

        {dependency_faulty, Package@1} ->
            dependency_faulty(Package@1)
    end.
...
explain(Error) ->
    _pipe = Error,
    _pipe@1 = explain_to_string(_pipe),
    gleam@io:print_error(_pipe@1).
