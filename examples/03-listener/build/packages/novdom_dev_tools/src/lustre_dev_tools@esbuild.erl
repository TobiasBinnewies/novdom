...
-export([download/2, bundle/3, bundle_node_modules/2]).
-export_type([package_json/0]).

-type package_json() :: {package_json, binary(), gleam@option:option(binary())}.

-spec module_main_file(binary()) -> {ok, binary()} |
    {error, gleam@json:decode_error()}.
module_main_file(Json) ->
    Dot_encoder = gleam@dynamic:field(
        <<"import"/utf8>>,
        fun gleam@dynamic:string/1
    ),
    Exports_decoder = gleam@dynamic:field(<<"."/utf8>>, Dot_encoder),
    Decoder = gleam@dynamic:decode2(
        fun(Field@0, Field@1) -> {package_json, Field@0, Field@1} end,
        gleam@dynamic:field(<<"main"/utf8>>, fun gleam@dynamic:string/1),
        gleam@dynamic:optional_field(<<"exports"/utf8>>, Exports_decoder)
    ),
    gleam@result:map(
        gleam@json:decode(Json, Decoder),
        fun(Package_json) -> case erlang:element(3, Package_json) of
                {some, Exports} ->
                    Exports;

                none ->
                    erlang:element(2, Package_json)
            end end
    ).

-spec get_module_main(binary()) -> {ok, binary()} |
    {error, lustre_dev_tools@error:error()}.
get_module_main(Name) ->
    Root = lustre_dev_tools@project:root(),
    Module = filepath:join(Root, <<"node_modules/"/utf8, Name/binary>>),
    Module_json = filepath:join(Module, <<"package.json"/utf8>>),
    case simplifile_erl:is_directory(Module) of
        {ok, false} ->
            {error, {dependency_not_found, Name}};
        ...
        {ok, true} ->
            gleam@result:'try'(
                begin
                    _pipe = simplifile:read(Module_json),
                    gleam@result:replace_error(_pipe, {dependency_faulty, Name})
                end,
                fun(Json) ->
                    gleam@result:'try'(
                        begin
                            _pipe@1 = module_main_file(Json),
                            gleam@result:replace_error(
                                _pipe@1,
                                {dependency_faulty, Name}
                            )
                        end,
                        fun(Main_file) ->
                            _assert_subject = filepath:expand(
                                filepath:join(Module, Main_file)
                            ),
                            {ok, Main} = case _assert_subject of
                                {ok, _} -> _assert_subject;
                                _assert_fail ->
                                    erlang:error(#{gleam_error => let_assert,
                                                message => <<"Assertion pattern match failed"/utf8>>,
                                                value => _assert_fail,
                                                module => <<"lustre_dev_tools/esbuild"/utf8>>,
                                                function => <<"get_module_main"/utf8>>,
                                                line => 132})
                            end,
                            {ok, Main}
                        end
...
bundle(Input_file, Output_file, Is_prod) ->
...
-spec bundle_node_modules(list(binary()), binary()) -> lustre_dev_tools@cli:cli(nil).
bundle_node_modules(Modules, Build_dir) ->
    lustre_dev_tools@cli:log(
        <<"Bundling all used node modules"/utf8>>,
        fun() ->
            lustre_dev_tools@cli:do(
                download(
                ...
                fun(_) ->
                    Dir = filepath:join(Build_dir, <<"modules"/utf8>>),
                    lustre_dev_tools@cli:'try'(
                        (gleam@list:try_each(
                            Modules,
                            fun(Module_name) ->
                                gleam@result:'try'(
                                    get_module_main(Module_name),
                                    fun(Module_main) ->
                                        Output_file = filepath:join(
                                            Dir,
                                            <<Module_name/binary, ".mjs"/utf8>>
                                        ),
                                        Options = [Module_main,
                                            <<"--bundle"/utf8>>,
                                            <<"--external:node:*"/utf8>>,
                                            <<"--format=esm"/utf8>>,
                                            <<"--tree-shaking=false"/utf8>>,
                                            <<"--outfile="/utf8,
                                                Output_file/binary>>],
                                        exec_esbuild(
                                            lustre_dev_tools@project:root(),
                        ...
                        fun(_) ->
                            lustre_dev_tools@cli:success(
                                <<<<"Bundled all node modules at `"/utf8,
                                        Dir/binary>>/binary,
                                    "`"/utf8>>,
                                fun() -> lustre_dev_tools@cli:return(nil) end
                            )
