...
var global_listener2 = global_listener;

// build/dev/javascript/novdom/novdom/hotkey.mjs
function init() {
  return global_listener2(onkeydown(keypress_callback));
}
...
function add_to_viewport(comp2, id) {
  const elem = comp2.element;
  const viewport = document.getElementById(id);
  viewport.appendChild(elem);
  execute_render(elem);
}
function create_element(tag, children) {
  const id = unique_id();
...
function create_text_element(value2) {
  const id = unique_id();
  const elem = document.createElement("p");
  elem.setAttribute("id", id);
  elem.textContent = value2;
  return new Component(id, elem);
}
function add_attribute(comp2, name, value2) {
  const elem = comp2.element;
...
function handle_attribute(elem, name, value2, remove) {
  ...
  switch (name) {
    case "tailwind":
      if (!remove) {
        elem.className = twMerge(elem.className, value2);
        return;
      }
...
function add_listener(comp2, name, callback) {
  const elem = comp2.element;
  elem.addEventListener(name, callback);
  return comp2;
}
function create_once(callback) {
  const fn = (event) => {
    callback(event);
...
function add_global_listener(name, callback) {
  document.addEventListener(name, callback);
}
function set_child(comp2, child_comp) {
  const elem = comp2.element;
...
function add_render(comp2, onrender) {
  const elem = comp2.element;
  elem.onrender = onrender;
  return comp2;
}
function add_unrender(comp2, onunrender, trigger) {
  const elem = comp2.element;
...
function global_listener(listener) {
...
function set_parameters(component, params) {
  each(
    params,
    (param) => {
      if (param instanceof Attribute) {
        ...
        return add_attribute(component, key, value2);
...
function div(parameters, children) {
  let _pipe = create_element(div_tag, children);
...
function utilize(state, do$) {
  ...
  let comp2 = create_element(state_component_tag, toList([children]));
  let callback = (a) => {
    let do$1 = do$(a);
    let _pipe = comp2;
...
function init3() {
  ...
  global_listener2(onmousemove((e) => {
  ...
  return add_to_viewport(_pipe, "_drag_");
}
      ...
      return div(
        ...
        toList([
          div(
            ...
            toList([create_text_element("Click me!")])
          )
        ])
      );
    }
